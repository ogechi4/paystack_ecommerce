...
function Login() {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            if (currentState === 'Sign Up') {
                ...
                if (response.data.success && response.data.verify) {  // Check if verification is required
                    setNeedsVerification(true) // Set verification flag
    ...
    useEffect(() => {
        if (needsVerification) {
            navigate('/tokenVerify')  // Redirect to verification page if verification is required
        } else if (token) {
            navigate('/')  // Redirect to home page if no verification needed
        }
    }, [needsVerification, token, navigate])
    return (
